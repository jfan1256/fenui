...
def generate_plot():
    ...
    To test local flask server run this:
        1. Start the flask server by cding to the directory that this file is located in and running this in powershell: python flask_generate_db.py
    ...
    try:
        fetch_data = FetchData(query=query, start_date=start_date_str, end_date=end_date_str, prod=prod, ngrok_host=ngrok_host, ngrok_port=ngrok_port)
    ...
    gen_index, gen_combine = generate_index.generate_index_db()
    print("-" * 60 + f"\nGenerated Dataframe: \n\n\n{gen_combine}")
